...
//AnoANO_OF.GetOneByte是初级数据解析函数
//串口每接收到一字节光流数据 调用本函数一次
//函数参数就是串口收到的数据
...
void AnoOF_GetOneByte(uint8_t data)
{
	...
	else if(state==5&&_data_len>0)	//有效数据
	{
		_data_len--;
	...
	else if(state==6)
	{
		...
		//接收到完整的一帧数据后 会自动调用数据解析函数ANO_OF_DataAnl
...
{
	ANO_OF.STATE.alt = (_ano_of_sta_enum)(state & (0x01));	//高度有效
	ANO_OF.STATE.of = (_ano_of_sta_enum)((state & (0x02))>>1);	//光流有效
	ANO_OF.STATE.alt_fus = (_ano_of_sta_enum)((state & (0x04))>>2);	//高度融合有效
	ANO_OF.STATE.of_fus = (_ano_of_sta_enum)((state & (0x08))>>3);	//光流融合有效
}
