...
uint8_t Thr_Push_Over_Deadband(void)
{
  ...
  thr_last=thr_now;
	...
	if(RC_NewData[0]<=Deadzone_Min) thr_now=0;			//当前油门低于中位死区->0
	else if(RC_NewData[0]<=Deadzone_Max) thr_now=1;	//当前油门位于中位死区->1
  else thr_now=2;  																//当前油门高于中位死区->2
	
	//当前油门低于中位死区 上次油门高于中位死区
  if(thr_now==0&&thr_last==1)       return push_flag=0;//中->低
	
  else if(thr_now==1&&thr_last==0)  return push_flag=1;//低->中
  
	else if(thr_now==2&&thr_last==1)  return push_flag=2;//中->高
  
	else if(thr_now==1&&thr_last==2)  return push_flag=3;//高->中
  
	else push_flag=0;//其他情况
