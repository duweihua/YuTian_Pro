     ...
     &&RC_NewData[0]<=Deadzone_Max)	//油门位于中位
  {
    ...
    if(High_Pos_Control_Cnt>=2)//竖直高度控制周期2*5=10ms
    {
      ...
      PID_Control(&Total_Controller.High_Position_Control);//高度位置控制器
			...
			//内环速度期望
      if(target_climb_alt==0) Yaw_Vel_Target=taret_climb_rate;//速度控制模式：速度期望来源于外部直接给定
      else Yaw_Vel_Target=Total_Controller.High_Position_Control.Control_OutPut;//位置控制模式：速度期望来源于位置控制器输出
			
#ifdef YAW_VEL_FEEDFORWARD  //速度前馈控制器(未开启)
    ...
    handmove_flag=FALSE;	//什么标志
    move_flag=0;	//？
  }
	
  else if(RC_NewData[0]>Deadzone_Max)//油门高于中位死区 给定上升速度期望
  {
    //油门杆上推 给定速度期望
...
//着陆过程中存在各种标志位 
//在一次飞行过程中可能会频繁切换 需要清空上次未进行完毕的标志位
void land_reset()
{
