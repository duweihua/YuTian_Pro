...
void POS_Ctrl(float dT_s)
{
	...
	if((FollowManager.ptrFrame->FormType == HorizontalLine)&&(FollowManager.ActionList == ActionGoForward))
	{
			exp_velocity[1] = -(Remote.roll - 1500) * 0.3f  + 0;
	}
	else
	{
...
//	{
//			fb_velocity[1] = 0;
//			fb_velocity_fixed[1] = 0;
//			pos_velocity_err_i[1] = 0;

//			if((exp_velocity[1] > 0.0f)||(exp_velocity_old[1] < 0.0f))
//			{
//					pos_velocity_err_i[1] = 0;
//			}
//			if((exp_velocity[1] < 0.0f)||(exp_velocity_old[1] > 0.0f))
...
//	{
			...
			exp_velocity_old[1] = exp_velocity[1];
